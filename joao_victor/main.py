...
def ListagemUsuarios():
    ...
    vFormPedido.close()
    vCursor = conn.vBanco.cursor()
    vComandoSQL = "SELECT * FROM PRODUTOS"
    vCursor.execute(vComandoSQL)
    vDadosRetornados = vCursor.fetchall()
...
def cadastroUsuario():
    ...
    try:
        ...
        if vCodigoUsuario != "" and vNome != "" and vSenha != "":
            vCursor.execute("INSERT INTO tab_usuario values ('"+vCodigoUsuario+"', '"+vNome+"', '"+vSenha+"')")
            conn.vBanco.commit()
...
def criar_menu(botao):
    ...
    # Opções fixas
    opcoes_fixas = [
        ("Cadastrar Produto", lambda: (vFormCadastroProduto.show(), vFormPedido.close())),
        ("Finalizar expediente", lambda: (vFormPedido.close(), vFormLogin.show()))
    ]
...
def limparEdits():   
    ...
    vFormCadastroProduto.edtCategoria.setText("")  
